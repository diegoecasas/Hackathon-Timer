...
interface TimerSetupProps {
  onSetupComplete: (phases: Phase[]) => void;
}

const TimerSetup: React.FC<TimerSetupProps> = ({ onSetupComplete }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSetupComplete(phases);
  };

  return (
    <div className="max-w-2xl mx-auto p-4 sm:p-8 bg-gray-900 text-white rounded-3xl shadow-2xl border-2 border-teal-500/50">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-extrabold text-teal-300">Configuración del Temporizador de Hackathon</h1>
        <p className="text-gray-400 mt-2">Define las etapas de tu hackathon.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-between items-center mt-6">
          <button type="button" onClick={addPhase} className="px-5 py-2 text-teal-300 border border-teal-400 rounded-lg hover:bg-teal-400/20 transition">
            Añadir Fase
          </button>
          <button type="submit" className="px-8 py-3 text-lg font-bold rounded-lg transition-all duration-300 bg-teal-500 text-white hover:bg-teal-400 focus:ring-4 focus:ring-teal-300 shadow-lg hover:shadow-xl">
            Iniciar Hackathon
          </button>
        </div>
      </form>
